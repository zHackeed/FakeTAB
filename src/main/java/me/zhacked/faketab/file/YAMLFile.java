package me.zhacked.faketab.file;
...
public class YAMLFile {
    ...
    public List<String> getColoredStringList(Object... path) {
        List<String> stringList = new ArrayList<>();
        try {
            this.configurationNode.node(path).getList(String.class).forEach(s -> stringList.add(ChatColor.translateAlternateColorCodes('&', s)));
            return stringList;
        } catch (SerializationException e) {
            e.printStackTrace();
            return Lists.newArrayList("No result for path: ", Arrays.toString(path));
